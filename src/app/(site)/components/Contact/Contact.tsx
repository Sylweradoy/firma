...
/* używamy globalnych styli przycisków (bez komponentów TSX, więc zero problemów z typem `as`) */
...
export default function Contact() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!name || !phone) {
      alert("Uzupełnij imię i numer telefonu.");
      return;
    }
    setSubmitting(true);
    try {
    ...
    >
      ...
      >
        ...
        <p className={styles.lead}>
          Najszybciej przez telefon lub formularz. Działamy w okolicach Jeleniej
          Góry i okolic.
        </p>
      </motion.header>
      ...
      >
        <motion.li className={styles.card} variants={cardV}>
          <a href="tel:+48600000000" className={styles.cardLink}>
            <span className={styles.icon}>
              <IPhone />
            </span>
            <span className={styles.cardTitle}>Telefon</span>
            <span className={styles.cardValue}>+48 600 000 000</span>
          </a>
        </motion.li>
        <motion.li className={styles.card} variants={cardV}>
      ...
      >
        <p>
          Wycena jest <strong>darmowa</strong> przy zleceniu realizacji —
          odliczamy ją od końcowego rozliczenia. Jeśli nie dojdzie do
          realizacji: koszt dojazdu i wyceny to <strong>100&nbsp;zł</strong>.
          Projekty (np. kuchni) rozliczamy osobno.
        </p>

        {/* Sekundarny guzik w 100% na globalnych klasach */}
        <ButtonSecondary onClick={() => setOpen(true)}>
          Zasady wyceny / szczegóły
        </ButtonSecondary>
      </motion.div>
      ...
      >
        ...
        <div className={styles.actions}>
          {/* Primary jako prawdziwy <button> z globalnych klas */}
        ...
        >
          ...
          <div className={styles.modalInner}>
            ...
            <ul className={styles.rulesList}>
              <li>
                <strong>Darmowa wycena</strong> przy zleceniu – odliczamy od
                końcowej kwoty.
              </li>
              <li>
                Bez realizacji: <strong>100&nbsp;zł</strong> za dojazd i wycenę.
              </li>
              <li>
                Projekty (np. kuchnia, zabudowy) – wycena i płatność{" "}
                <strong>osobno</strong>.
              </li>
              <li>
                Po akceptacji oferty ustalamy <strong>termin</strong> i
                podpisujemy <strong>umowę</strong>.
              </li>
            </ul>

            <div className={styles.modalActions}>
              {/* Sekundarny guzik – globalne klasy */}
              <ButtonSecondary ref={closeBtnRef} onClick={() => setOpen(false)}>
                Zamknij
              </ButtonSecondary>
            </div>
          </div>
